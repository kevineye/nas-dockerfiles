...
if [ ! -e /.initialized ]; then
    ...
    if [ -z $AFPD_SIZE_LIMIT ]; then
        echo "/share \"${AFPD_NAME}\" allow:$AFPD_LOGIN rwlist:$AFPD_LOGIN fperm:664 dperm:775 cnidscheme:dbd options:usedots,uperm,tm" > /etc/netatalk/AppleVolumes.default
    else
        echo "/share \"${AFPD_NAME}\" allow:$AFPD_LOGIN rwlist:$AFPD_LOGIN fperm:664 dperm:775 cnidscheme:dbd options:usedots,uperm,tm volsizelimit:$AFPD_SIZE_LIMIT" > /etc/netatalk/AppleVolumes.default
    fi
